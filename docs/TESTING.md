...
- The same verified WSS lifecycle with 350 ms and 450 ms simulated propagation each way (700 ms and 900 ms added full-path RTT), sustaining a held drag for ten seconds. The 900 ms route also pauses wire delivery for 750 ms, preserving FIFO order, then drops only application heartbeats while relay pings/challenges/probes still flow. Holds release, both sockets stay connected, and reactivation is explicit. Exact deadline boundaries use a deterministic clock. These are not measurements on the user's network.
- Reported heartbeat ages of 900 ms and 927 ms, bounded automatic RTT timing, controller update gaps, paused calibration, repeated-nonce rejection, expired input, a three-second maximum heartbeat-silence bound and old-epoch cleanup. Input and probes cannot substitute for a heartbeat.
...
The pre-fix source reproduced `Paused: stale heartbeat` at a measured token age of 903 ms on the 900 ms simulated route, matching the owner's 900/927 ms failures. The suite also covers update gaps beyond the old controller cutoff. The timeout tradeoff is explicit: network silence now permits 2–3 seconds before cleanup; the 100 ms local queue limit and native capture watchdogs are unchanged.
...
| Freeze controller | Suspend its process while holding a key/button, leaving the relay alive. Target releases after its current 2–3 second network-silence limit from the last accepted heartbeat, plus scheduling time; in-flight heartbeats can add transit time after suspension. Resume; control must be reactivated. |
