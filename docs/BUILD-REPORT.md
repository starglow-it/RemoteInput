...
Source: commit `fc76b556c830fb393fe9e51c685f6363ffe8811e`, tree `b06225bc8a23a6499f453d5958c2c6d2f6bfb975`.

[Successful verification and native packaging run](https://github.com/starglow-it/RemoteInput/actions/runs/34890902331).

Includes the Windows activation fix: native capture transitions run on the hook thread, busy hook callbacks count as progress, and automatic pauses explain their local cause. See [immediate-pause troubleshooting](TROUBLESHOOTING.md#controlling-changes-immediately-to-paused-on-windows). The existing target and relay remain compatible with the updated Windows controller.
...
|---|---|---|
| Windows x64 | Target and controller | [Windows package](https://github.com/starglow-it/RemoteInput/actions/runs/34890902331/artifacts/10366747791) |
| macOS Intel | Target | [Intel Mac package](https://github.com/starglow-it/RemoteInput/actions/runs/34890902331/artifacts/10366965593) |
| macOS Apple Silicon | Target | [Apple Silicon Mac package](https://github.com/starglow-it/RemoteInput/actions/runs/34890902331/artifacts/10367270364) |
...
- Windows: **72 tests passed**. The packaged binary loaded Win32, reported desktop access on the runner, loaded certificate roots, and confirmed an embedded relay address.
- Intel and Apple Silicon Macs: **71 tests passed, one Windows-only DPAPI test skipped** on each. Both packaged binaries loaded CoreGraphics and certificate roots and confirmed an embedded relay address.
...
- Hook activation regressions use simulated Win32 APIs; the delayed WSS lifecycle uses 225 ms simulated propagation each way with FIFO delivery. The suite verifies that a capture failure releases held input and that reactivation is explicit. These simulations do not measure physical desktop behavior or the owner's network latency.
...
Earlier [setup-pending validation builds](https://github.com/starglow-it/RemoteInput/actions/runs/34872458151) did not embed a relay address. The [first configured builds](https://github.com/starglow-it/RemoteInput/actions/runs/34887713919) predate the Windows activation fix. Use the current downloads above for everyday use.
